...
#read in data
myData<- readRDS(paste(myfolder,"Fulldata allgroups.rds",sep="/"))
...
good_comparisons_groups<-read.csv(paste(myfolder,"comparison_jobs_groups.csv",sep="/"))
ok_comparisons_groups<-  read.csv(paste(myfolder,"comparison_jobs_groups_less_good.csv",sep="/"))
comparisons <- rbind(good_comparisons_groups, ok_comparisons_groups)
comparisons<- arrange(comparisons, desc(Realm), desc(Datasets))

nrow(comparisons)#143
...
#pltQltyCheck<- dcast(mydataAggSubset, Realm + Plot_ID + Year ~ commonGroup, value.var = "Number",  fill = -999 , sum)
...
for(i in 1:length(plts)){
  ...
  prior1 = c(set_prior("normal(0,0.1)", class = "b"))
  ...
  write.table(data.frame(Plot_ID = plts[i]),
              file = paste0("output_" ,task.id,  realm, "_" ,Taxon1, "_" ,Taxon2, ".txt"),
              sep = "\t",
              append = TRUE,
              row.names = FALSE)
...
Wcor_samples <- sapply(1:1000,function(i){
...
png(paste0("histogram_",  task.id,   realm, "_" , Taxon1, "_" ,Taxon2, ".png"), width = 350, height = 650)
par(mfrow= c(2,1))
hist(Wcor_samples, xlim = c(-1, 1), main = " weighted correlation")
