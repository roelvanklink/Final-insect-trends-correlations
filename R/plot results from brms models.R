...
dataOrders<- readRDS("/Data/Fulldata allorders.rds")
dataGroups<- readRDS("/Data/Fulldata allgroups.rds")
...
filenamesCorraw <- list.files("/model-outputs//prior sd1/", pattern="cors_*", full.names=TRUE)
...
# get some elementary info on the data : 
...
# calculate mean and sd of slope estimates
...
cor(mnSlopes$sdTrend_T1, mnSlopes$sdTrend_T2) # 0.866 high correlation so we might just use the mean sd of the two trends for weighting later
...
cor.test(mnSlpFw$trend_T1, mnSlpFw$trend_T2) # 
cor.test(mnSlpT$trend_T1,  mnSlpT$trend_T2) #
# try weighted correlation 

weightedCorr(mnSlpFw$trend_T1, mnSlpFw$trend_T2 , weights = 1/mnSlpFw$mnSD) # 
weightedCorr(mnSlpT$trend_T1, mnSlpT$trend_T2 , weights = 1/mnSlpT$mnSD) # 
...
hist(slopes$trend_T2) # Almost equal increases and decreases, but parallel increases and declines are more common 
...
while (nrow(mnSlopesX)>0) {
  ...
  mnSlopesRandom<- rbind(mnSlopesRandom, mnSlopesRandom2); dim(mnSlopesRandom) # glue selected dfs  together
  mnSlopesX<- mnSlopesX [!mnSlopesX$index1 %in% mnSlopesRandom$index1 &  # remove the randomly selected taxa from each plot 
                        ...
                        !mnSlopesX$index2 %in% mnSlopesRandom$index2, ] ; dim(mnSlopesX) #  until there's nothing left 
}; dim(mnSlopesRandom)
...
cor.test(mnSlopesRandomFw$trend_T1,mnSlopesRandomFw$trend_T2) # 
...
cor.test(mnSlopesRandomT$trend_T1, mnSlopesRandomT$trend_T2) # 
...
# Is the predictability higher in the presence of a trend? 
...
cor.test(subset(noSlope80, Realm == "Freshwater")$medianSlpT1, subset(noSlope80, Realm == "Freshwater")$medianSlpT2) # 
cor.test(subset(noSlope80, Realm == "Terrestrial")$medianSlpT1, subset(noSlope80, Realm == "Terrestrial")$medianSlpT2) # -
...
sameDirection80<- aggregate (.~ Var3, data = subset(test, Var1 == Var2  ), sum); sameDirection80#
...
cor.test(subset(sigSlope80, Realm == "Freshwater")$medianSlpT1, subset(sigSlope80, Realm == "Freshwater")$medianSlpT2) # 
cor.test(subset(sigSlope80, Realm == "Terrestrial")$medianSlpT1, subset(sigSlope80, Realm == "Terrestrial")$medianSlpT2) # 
...
test <- as.data.frame(table(sigSlope80both$medianSlpT1 >0, sigSlope80both$medianSlpT2>0, sigSlope80both$Realm ))# 
test$prop <- test$Freq/rep(table(sigSlope80$Realm), each = 4)
oppositeDirection80sig<- aggregate (.~ Var3, data = subset(test, Var1 != Var2  ), sum); oppositeDirection80sig # 
...
cor.test(subset(sigSlope95, Realm == "Freshwater")$medianSlpT1, subset(sigSlope95, Realm == "Freshwater")$medianSlpT2) #
cor.test(subset(sigSlope95, Realm == "Terrestrial")$medianSlpT1, subset(sigSlope95, Realm == "Terrestrial")$medianSlpT2) # 
...
sameDirection95<- aggregate (.~ Var3, data = subset(test, Var1 == Var2  ), sum); sameDirection95 #
...
filenamesCorraw <- list.files("model-outputs/rest taxa prior sd1/", pattern="cors_*", full.names=TRUE)
...
# Fig S2 plot correlations for by  taxon 
...
# excursion: why is the variance in the freshwater so much higher than in Terrestrial ? 
